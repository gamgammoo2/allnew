...
result = soup.select('a[href$=".com"]') #href$=는 .com 으로 끝나는 것
for item in result:
    print(item['href'])

result = soup.select('a[href*="daum"]') #href*=는 daum을 포함하는 것
for item in result:
    print(item['href'])
...
li = soup.find_all(href = re.compile(r"^https://")) #r은 row string을 나타냄
